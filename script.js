...
function createNote() {
  let notes = localStorage.getItem("notes");//Using local storage to store and retrieve data
...
${noteText}</textarea
    ...
    row.innerHTML += htmlCode;//Manipulating the DOM by updating innerHTML of elements
  } else {
  ...
  notesObj.push(myObj);
  localStorage.setItem("notes", JSON.stringify(notesObj));//Using local storage to store and retrieve data:
  showNotes();
}
...
${element.text}</textarea>
  ...
  if (notesObj.length != 0) {
    row.innerHTML = html;//Manipulating the DOM by updating innerHTML of elements
  } else {
    row.innerHTML = `<h4>No Notes Yet! Please add a note. <img src="arrow.png" width="50"/></h4>`;//Manipulating the DOM by updating innerHTML of elements
  }
}
